...
When Language Server Protocol support is enabled for this file type, compiler diagnostics for the new content are appended to the response (\`<lsp_diagnostics>\`). Absence of diagnostics is NOT a guarantee the change compiles — slow language servers (notably pyright on cold start) can miss the deadline, in which case a "timed out" note appears. Treat "timed out" as "unknown", not "clean".`,
    parametersJsonSchema: {
      type: 'object',
      properties: {
        [PARAM_FILE_PATH]: {
...
When Language Server Protocol support is enabled for this file type, compiler diagnostics for the edited content are appended to the response (\`<lsp_diagnostics>\`). Absence of diagnostics is NOT a guarantee the change compiles — slow language servers (notably pyright on cold start) can miss the deadline, in which case a "timed out" note appears. Treat "timed out" as "unknown", not "clean".`,
    parametersJsonSchema: {
      type: 'object',
      properties: {
        [PARAM_FILE_PATH]: {
